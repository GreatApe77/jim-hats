...
class GymChallengesRepository {
  ...
  Future<List<GymChallenge>> getGymChallengesOfUser(int userId)async{
    ...
    List<GymChallenge>? challenges = MemoryCache.get<List<GymChallenge>>('challenges');
    if(challenges==null){
    await Future.delayed(Duration(seconds: 2));
    challenges = await _gymChallengeDataSource.getGymChallengesOfUser(userId);
    MemoryCache.store<List<GymChallenge>>('challenges', challenges,duration: Duration(minutes: 1));
    }
